...
>
> 🎯 **核心功能**: 源码级中文文档 | 原生OpenAI支持 | Google AI全面集成 | 自定义端点配置 | 智能模型选择 | 多LLM提供商支持 | 模型选择持久化 | Docker容器化部署 | 专业报告导出 | 完整A股支持 | 中文本地化

基于多智能体大语言模型的**中文金融交易决策框架**。本项目在原版基础上进行了深度优化，为中文用户提供了完整的 A 股/港股/美股分析能力，并**对所有源代码添加了全面的中文文档**，极大地降低了二次开发的门槛。
...
**🎯 我们的使命**: 为中国用户提供完整的中文化体验和开发者文档，支持 A 股/港股市场，集成国产大模型，推动 AI 金融技术在中文社区的普及应用。
...
为了方便中文开发者进行二次开发和深入研究，我们为项目的**每一个** Python 源代码文件、类和函数都添加了详尽的中文文档字符串 (Docstrings)。

- **全覆盖**: 100% 的模块、类和函数均包含中文文档。
- **高质量**: 文档清晰地解释了代码的目的、参数和返回值。
- **降低门槛**: 大幅降低了理解和修改代码的难度。

开发者可以直接在 IDE 中查看每个组件的中文说明，快速上手。

## 🆚 中文增强特色

**相比原版新增**: 源码级中文文档 | 智能新闻分析 | 多层次新闻过滤 | 新闻质量评估 | 统一新闻工具 | 多LLM提供商集成 | 模型选择持久化 | 快速切换按钮 | | 实时进度显示 | 智能会话管理 | 中文界面 | A股数据 | 国产LLM | Docker部署 | 专业报告导出 | 统一日志管理 | Web配置界面 | 成本优化
